...
void main() {
  runApp(
      FacultyPage()); // Entry point of the application, runs FacultyPage widget
}
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment:
              CrossAxisAlignment.stretch, // Align elements to stretch across
          children: [
            ...
            DashboardCard(
              ...
              widgets: [
                ...
                StatWidget(
                  ...
                  countColor:
                      Colors.orange, // Orange color for yet to pay stats
                  onStatClicked: () {},
                ),
              ],
...
class AbsenteesPage extends StatelessWidget {
  ...
  AbsenteesPage(
      {required this.absentees}); // Constructor accepting the absentees list
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return ListTile(
            ...
            subtitle:
                Text('Roll No: ${student['rollNo']}'), // Display roll number
          );
        },
      ),
    );
        ...
        itemBuilder: (context, index) {
          return Padding(
            padding: const EdgeInsets.symmetric(
                vertical: 8.0, horizontal: 16.0), // Padding for each item
            child: Container(
...
class SectionSelectionPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title:
            Text('Select Section for $year'), // AppBar title with selected year
        leading: IconButton(
          icon: Icon(Icons.arrow_back), // Back button
          onPressed: () {
            Navigator.pop(context); // Go back on pressing the button
                    ...
                    MaterialPageRoute(
                      builder: (context) => SectionDashboard(
                        ...
                        students: List.generate(60, (index) {
                          ...
                          return {
                            'name':
                                'Student ${index + 1}', // Example student name
                            'rollNo': 'REG${index + 1}', // Example roll number
                            'tuitionFees':
                                20000 + (index * 500), // Example tuition fees
                            'busOrHostelFees':
                                5000 + (index * 200), // Example bus/hostel fees
                            'totalFees': 20000 +
                                (index * 500) +
                                5000 +
                                (index * 200), // Total fees
                            'totalPaid':
                                15000 + (index * 400), // Example total paid
                            'yetToPay': (20000 +
                                    (index * 500) +
                                    5000 +
                                    (index * 200)) -
                                (15000 +
                                    (index * 400)), // Yet to pay calculation
                          };
                        }),
                      ),
                    ),
...
class SectionDashboard extends StatefulWidget {
  ...
  SectionDashboard(
      {required this.year, required this.section, required this.students});
...
class _SectionDashboardState extends State<SectionDashboard> {
  ...
  void _updateStudentFees(int index, double newTotalPaid) {
    setState(() {
      ...
      widget.students[index]['yetToPay'] =
          widget.students[index]['totalFees'] - newTotalPaid;
    });
  }
  ...
  void _showEditDialog(int index) {
    ...
    TextEditingController totalPaidController =
        TextEditingController(text: student['totalPaid'].toString());
        ...
        return AlertDialog(
          ...
          actions: <Widget>[
            ...
            TextButton(
              ...
              onPressed: () {
                double newTotalPaid =
                    double.tryParse(totalPaidController.text) ??
                        student['totalPaid'];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
            '${widget.section} - ${widget.year}'), // Display the section and year
        leading: IconButton(
          icon: Icon(Icons.arrow_back), // Back button
          onPressed: () {
            Navigator.pop(context); // Go back on pressing the button
          ...
          Expanded(
            ...
            child: SingleChildScrollView(
              scrollDirection: Axis
                  .horizontal, // Enable horizontal scrolling for wider tables
              child: DataTable(
                columns: [
                  DataColumn(label: Text('Roll No')), // Column for Roll Number
                  DataColumn(label: Text('Name')), // Column for Student Name
                  DataColumn(
                      label: Text('Tuition Fees')), // Column for Tuition Fees
                  DataColumn(
                      label: Text(
                          'Bus/Hostel Fees')), // Column for Bus/Hostel Fees
                  DataColumn(
                      label: Text('Total Fees')), // Column for Total Fees
                  DataColumn(
                      label: Text('Total Paid')), // Column for Total Paid
                  DataColumn(
                      label: Text('Yet to Pay')), // Column for Yet to Pay
                ...
                rows: widget.students.map((student) {
                  ...
                  return DataRow(cells: [
                    DataCell(
                        Text(student['rollNo'].toString())), // Roll number cell
                    DataCell(Text(student['name'].toString())), // Name cell
                    DataCell(Text(
                        '₹${student['tuitionFees'].toString()}')), // Tuition fees cell
                    DataCell(Text(
                        '₹${student['busOrHostelFees'].toString()}')), // Bus/Hostel fees cell
                    DataCell(Text(
                        '₹${student['totalFees'].toString()}')), // Total fees cell
                    DataCell(Text('₹${student['totalPaid'].toString()}',
                        style:
                            TextStyle(color: Colors.green))), // Total paid cell
                    DataCell(Text('₹${student['yetToPay'].toString()}',
                        style: TextStyle(
                            color: Colors.orange))), // Yet to pay cell
                    DataCell(
                      IconButton(
